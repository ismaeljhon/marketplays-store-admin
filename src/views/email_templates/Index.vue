<template>
    <v-card>
        <v-card-title>
            <v-row dense>
                <v-col cols="5">
                    <h3 class="mr-2"><v-icon left>mdi-email-newsletter</v-icon>Email Templates</h3>
                </v-col>
                <v-col cols="7" class="text-right">
                    <download-csv class="mr-2 v-btn v-btn--depressed v-btn--flat v-btn--outlined v-btn--tile theme--light v-size--small" :data="emailTemplates" style="cursor: pointer">
                         <v-icon left>mdi-download</v-icon>Export Email Templates
                    </download-csv>
                ...
                <template slot="item.action" slot-scope="row">
                    <v-tooltip top>
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn small icon v-bind="attrs" v-on="on" color="primary" @click.prevent="$refs.emailTemplateFormModal.show(row.item, false)">
